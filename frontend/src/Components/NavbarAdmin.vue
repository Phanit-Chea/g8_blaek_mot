...
const login = async () => {
  ...
  try {
    const response = await useAuth.login(formData.value.email, formData.value.password)
    if (response.success) {
      showLoginPopup.value = false
...
.popup .login-popup .form-group {
  margin-bottom: 16px;
}

.popup .login-popup .submit {
  width: 100%;
  background-color: #3ea009;
  color: white;
  border: none;
  padding: 8px;
  border-radius: 4px;
...
.popup .login-popup .error-message {
  color: red;
  font-size: 0.875rem;
}
</style>
